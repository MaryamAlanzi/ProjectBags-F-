...
export default function Navbar({ token, setToken, Admin, setAdmin }) {
 ...
 {Admin == "Maryam.com" ? (
            <Link to="/adds">أضافه شنط السفر </Link>
          ) : (
            ""
          )}
          ...
          {Admin == "Maryam.com" ? <Link to="/Bags">شنط </Link> : ""}

          {Admin != "Maryam.com" ?
           <Link to="/Cart"> سله التسوق </Link> : ""}
          {Admin != "Maryam.com" ?<Link to="/TravalBags"> شنط السفر </Link>:""}
          {Admin != "Maryam.com" ?  <Link to="/UserBag"> شنط </Link>:""}
